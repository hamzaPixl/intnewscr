function ConfigReader(moduleName, moduleLocator, configRoutePath, valueRoutePath) {
    ...
    this.configRoutePath = configRoutePath || './../../modules/';
...
ConfigReader.prototype = {
    ...
    configIsValid: function configIsValid() {
        ...
        let configCheck = this.getModuleConfigurationDefinition();
        let configValue = this.getModuleConfigurationValue();

        if (!configValue && !configCheck) {
            return false;
        }

        configCheck = configCheck['config'];

        for (var key in configCheck) {
            var ob = configCheck[key];
            if (ob.require) {
                if (configValue[ob.name] == undefined || typeof(configValue[ob.name]) !== ob.type) {
                    return false;
                }
            }
        }
    ...
    getServices: function getServices() {
        let config = this.getModuleConfigurationDefinition();
        if (!config) {
            return null;
        }
    ...
    getModuleConfigurationValue: function getModuleConfigurationValue() {
        ...
        let path = this.valueRoutePath + this.moduleLocator.getModule(this.moduleName).value;
        return require(path);
    },
    /**
    ...
    getModuleConfigurationDefinition: function getModuleConfigurationDefinition() {
        ...
        let path = this.configRoutePath + this.moduleLocator.getModule(this.moduleName).check;
        return require(path);
    },
